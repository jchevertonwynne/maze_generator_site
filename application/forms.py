...
class IntegerRequired(DataRequired):
    def __call__(self, form, field):
        if not field.data or isinstance(field.data, str) and not field.data.strip() or field.data > 200:
            message = field.gettext('This field only alllows integer values up to 200')
            field.errors[:] = []
